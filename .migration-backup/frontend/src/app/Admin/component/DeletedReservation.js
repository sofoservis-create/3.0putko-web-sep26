...
const DeletedReservation = () => {
  ...
  const { data, loading, error } = useFetchData(`${process.env.NEXT_PUBLIC_BASE_URL}/reservation/deleted`); // Fetch reservation data
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/reservation/delete/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleRestore = async (id) => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/reservation/restore/${id}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
