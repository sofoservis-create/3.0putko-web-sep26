...
export default function ClientPage() {
  ...
  const fetchComments = async (blogId) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/blog-comments/blog/${blogId}`);
      if (!response.ok) {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/blog/slug/${params.slug}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const incrementView = async () => {
      if (blog?._id) {
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/blog/${blog._id}/view`, {
          method: "POST",
        });
      }
    };
