...
useEffect(() => {
  ...
  const fireLeadEvent = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/facebook-events`, { // Replace with your actual backend URL
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        eventName: 'Lead',
  ...
  if (window.fbq && userData) {
    ...
    // Also send via CAPI to your backend
    try {
      await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/facebook-events`, { // Replace with your actual backend URL
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
