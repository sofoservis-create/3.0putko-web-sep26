...
const LocationInput = ({
  ...
  const { city } = useContext(FormContext);
  ...
  return (
    <div className={`relative flex ${className}`} ref={containerRef}>
      <div
        ...
        className={`flex z-10 flex-1  relative [ nc-hero-field-padding ] flex-shrink-0 items-center cursor-pointer focus:outline-none text-left ${
          showPopover ? "nc-hero-field-focused" : ""
        }`}
      >

        <div className="flex-grow min-w-0">
          <span className="block mb-0.5 font-semibold text-[9px] md:text-[11px] lg:text-[12.8px] leading-[20.48px] tracking-[0.64px] uppercase text-[#64748B] align-middle whitespace-nowrap">
            {desc}
          </span>
          <input
            className="block w-full p-0 font-semibold text-[#64748B] bg-transparent border-none focus:ring-0 focus:outline-none focus:placeholder-neutral-300 text-xs md:text-sm xl:text-lg"
