...
const SectionGridHasMap = () => {
  ...
  const hasMapApiKey = !!(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY);
  ...
  const mapAvailable = isLoaded && hasMapApiKey && !mapAuthError;
  ...
  const { data: searchResponse, loading, error } = useFetchData(
    cityReady
      ? `${process.env.NEXT_PUBLIC_BASE_URL}/accommodations/searching?${paginatedParams}`
      : null
  );

  const { data: pinsResponse } = useFetchData(
    cityReady
      ? `${process.env.NEXT_PUBLIC_BASE_URL}/accommodations/searching?${mapParams}`
      : null
  );
  ...
  const suggestNearbyAreas = (searchTerm) => {
    ...
    for (const [city, nearby] of Object.entries(nearbyMap)) {
      if (normalized.includes(city)) return nearby;
    }
  ...
  return (
    <div>
      <div className="relative flex min-h-screen">
        <div className="min-h-screen w-full xl:w-[60%] 2xl:w-[60%] max-w-[1940px] flex-shrink-0 px-4 md:px-6 lg:px-8 xl:pl-10 xl:pr-10">
          {/* Results context strip */}
          {displayStays.length > 0 && (
            <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 mb-5 font-inter">
          ...
          <div
            id="listing-section"
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-x-5 2xl:gap-x-6 gap-y-8"
          >
            {loading ? (
              ...
              <div className="mt-12 col-span-full flex flex-col items-center justify-center text-center px-6 text-neutral-700">
                <p className="text-xl font-semibold mb-2">
                  {language === "sk"
                    ? `Žiaľ, momentálne nemáme ubytovanie v oblasti „${city}“.`
                    : `We don’t have accommodations in “${
                        city || "this area"
                      }” yet.`}
                </p>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {suggestNearbyAreas(city).map((area) => (
                    <button
                      key={area}
                      onClick={() => updateCity(area.toLowerCase())}
        ...
        {!showFullMapFixed && (
          ...
          >
            <Map size={20} className="text-lg text-white" />
            <span>{t.Showmap}</span>
          </div>
        )}
        <div
            ...
            <div className="relative w-full h-full rounded-md overflow-hidden">
              ...
              {mapAvailable && (
                <div style={{ width: "100%", height: "100%" }}>
                  ...
                  >
                    {/* ===== Only render visible pins (first 40 instantly + progressive) ===== */}
                    {mapPins
                      .slice(0, visiblePinCount)
                      .map((item, index) => renderPriceTag(item, index))}
                  </GoogleMap>
                </div>
              )}
            </div>
