...
const DeletedAccommodation = () => {
  ...
  const { data, loading, error, refetch } = useFetchData(
    `${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/accommodation/deleted`
  );
  ...
  const handleRestore = async (id) => {
    try {
      const response = await fetch(
        `${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/accommodation/restore/${id}`,
  ...
  const handleDeletePermanently = async (id) => {
    ...
    try {
      const response = await fetch(
        `${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/accommodation/deleted/${id}`,
        { method: "DELETE" }
      );
