...
const Hosts = () => {
  ...
  const { data, loading, error } = useFetchData(`${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/hosts`);
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/hosts/${id}`, {
        method: 'DELETE',
        headers: {
