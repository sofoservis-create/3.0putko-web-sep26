...
export default function CancelBookingButton({
  ...
  const openDialog = async () => {
    ...
    try {
      const res = await apiFetch(
        `${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/cancellation/preview/${reservationId}`,
        { reservationId }
      );
  ...
  const confirm = async () => {
    ...
    try {
      const res = await apiFetch(
        `${(import.meta.env.VITE_API_URL || "https://backend-9k3q.onrender.com/api")}/cancellation/${reservationId}`,
        {
          method: "POST",
          reservationId,
