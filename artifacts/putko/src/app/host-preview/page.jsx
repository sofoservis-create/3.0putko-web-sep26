...
export default function HostWorkspace() {
  ...
  const refreshSetup = () => setSetupRefreshKey((key) => key + 1);
  ...
  useEffect(() => {
    ...
    listHostAccommodations()
      .then((result) => {
        if (!cancelled) setHostAccommodations(result.accommodations || []);
      })
      .catch(() => {
        if (!cancelled) setHostAccommodations([]);
      });
    return () => {
      cancelled = true;
    };
  }, [section, editorOpen, setupRefreshKey]);
