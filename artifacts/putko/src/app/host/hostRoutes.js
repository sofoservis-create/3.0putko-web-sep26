 ...
 * Returns `{ section, listingId, review, step }` or `null` when the path is
 * not a known Host screen so the workspace can redirect to the Today view.
 * `section` is one of today | listings | listing | calendar | menu; the
 * calendar section carries `listingId` when it is property-scoped.
 */
export function resolveHostLocation(pathname, search = "") {
  ...
  const base = { listingId: null, review: false, step: null };
  ...
  if (listingMatch) {
    ...
    return listingId ? { section: "listing", listingId, review, step } : null;
  }
