...
const LocationInput = ({
  ...
  const loadGoogleMapsScript = () => {
    return new Promise((resolve, reject) => {
      ...
      const scriptUrl = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(googleMapsApiKey)}&libraries=places`;
  ...
  const fetchSuggestions = async (input) => {
    ...
    try {
      ...
      const googlePromise = googleMapsApiKey
        ? (async () => {
  ...
  return (
    <div className={`relative flex ${className}`} ref={containerRef}>
      <div
        ...
        className={`relative z-10 flex min-w-0 flex-1 items-center space-x-3 [nc-hero-field-padding] cursor-pointer text-left focus:outline-none ${
          showPopover ? "nc-hero-field-focused" : ""
        }`}
      >
        <div className="flex h-9 w-9 shrink-0 items-center justify-center text-[#789087]">
