...
const TabFilters = ({ hideQuickTypes = false }) => {
  ...
  const {
    ...
    BathroomAmenities: appliedBathroomAmenities,
    updateKitchenDiningAmenities,
    KitchenDiningAmenities: appliedKitchenDiningAmenities,
    updateHeatingCoolingAmenities,
    HeatingCoolingAmenities: appliedHeatingCoolingAmenities,
    updateSafetyAmenities,
    SafetyAmenities: appliedSafetyAmenities,
    updateWellnessAmenities,
    WellnessAmenities: appliedWellnessAmenities,
    updateOutdoorAmenities,
    OutdoorAmenities: appliedOutdoorAmenities,
    updateParkingFacilities,
    ParkingFacilities: appliedParkingFacilities,
    updateCheckInOptions,
    CheckInOptions: appliedCheckInOptions,
    updateMeals,
    Meals: appliedMeals,
    updatePets,
    Pets: appliedPets,
    ...
    PartyOrganizing: appliedPartyOrganizing,
  ...
  const activeFilterCount = [
    hasSelection(drop),
    Number(pricemins) > 0 || (Number(pricemaxs) > 0 && Number(pricemaxs) < 2000),
    Number(Beds) > 0 || Number(Bedss) > 0 || Number(Bathroomss) > 0,
    hasSelection(appliedServices),
    hasSelection(appliedBathroomAmenities),
    hasSelection(appliedKitchenDiningAmenities),
    hasSelection(appliedHeatingCoolingAmenities),
    hasSelection(appliedSafetyAmenities),
    hasSelection(appliedWellnessAmenities),
    hasSelection(appliedOutdoorAmenities),
    hasSelection(appliedParkingFacilities),
    hasSelection(appliedCheckInOptions),
    hasSelection(appliedMeals),
    hasSelection(appliedPets),
    hasSelection(appliedPartyOrganizing),
    hasSelection(smoking),
  ].filter(Boolean).length;

  const moreFiltersLabel = activeFilterCount
    ? `${t.Morefilters} (${activeFilterCount})`
    : t.Morefilters;
  ...
  const renderTabMoreFilter = () => {
    ...
    return (
      <div>
        <button
          type="button"
          className={`flex min-h-11 items-center justify-center px-4 py-2 text-sm rounded-full border transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#238869] focus-visible:ring-offset-2 ${activeFilterCount ? "border-[#2C8360] bg-[#2C8360]/5 text-[#2C8360]" : "border-neutral-200 bg-white text-neutral-700 hover:border-[#2C8360] hover:text-[#2C8360]"}`}
              ...
              <div className="inline-flex flex-col w-full h-full max-w-4xl overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                <div className="relative flex-shrink-0 px-6 py-4 text-center border-b border-neutral-200 ">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full text-neutral-700 hover:bg-neutral-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#238869]"
                    aria-label="Zavrieť"
                  >
                    <X className="h-6 w-6" />
                  </button>
  ...
  const renderTabMoreFilterMobile = () => {
    ...
    return (
      <div className="w-full lg:hidden">
        ...
        <div className="grid grid-cols-2 gap-3 px-0.5 py-1 lg:hidden">
          {/* More Filters Button */}
          <button
            type="button"
            className={`flex min-h-11 w-full items-center justify-center px-4 py-2 text-sm rounded-full border transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#238869] focus-visible:ring-offset-2 ${activeFilterCount ? "border-[#2C8360] bg-[#2C8360]/5 text-[#2C8360]" : "border-neutral-200 bg-white text-neutral-700 hover:border-[#2C8360] hover:text-[#2C8360]"}`}
          ...
          <div className="relative w-full">
            <button
              className={`flex min-h-11 w-full items-center justify-center px-4 py-2 text-sm border rounded-full focus:outline-none transition-colors ${showSortingOptions ? "border-[#2C8360] bg-[#2C8360]/5 text-[#2C8360]" : "border-neutral-200 text-neutral-700 hover:border-[#2C8360] hover:text-[#2C8360]"}`}
              onClick={handleSortingClick}
              aria-expanded={showSortingOptions}
              aria-haspopup="menu"
            >
            ...
            {showSortingOptions && (
              <div className="absolute z-50 w-40 mt-2 bg-white rounded-2xl shadow-lg top-full border border-neutral-100">
                <ul className="py-2" role="menu">
                  <li role="none">
                    <button type="button" role="menuitem" className="w-full px-4 py-2 text-left hover:bg-gray-100 focus:bg-gray-100 focus:outline-none" onClick={() => handleSortOption("lowToHigh")}>
                      {t.LowtoHigh}
                    </button>
                  </li>
                  <li role="none">
                    <button type="button" role="menuitem" className="w-full px-4 py-2 text-left hover:bg-gray-100 focus:bg-gray-100 focus:outline-none" onClick={() => handleSortOption("highToLow")}>
                      {t.HightoLow}
                    </button>
                  </li>
                </ul>
        ...
        {(() => {
          ...
          return (
            <div
              aria-hidden={hideQuickTypes}
              className={`flex gap-2 overflow-x-auto px-0.5 no-scrollbar transition-all duration-200 ease-out lg:hidden ${
                hideQuickTypes
                  ? "pointer-events-none max-h-0 overflow-y-hidden py-0 opacity-0"
                  : "max-h-12 pt-2 pb-1 opacity-100"
              }`}
            >
              {chips.map(({ label, key }) => {
                const selectedKeys = Array.isArray(drop) ? drop : drop ? [drop] : [];
                const isActive =
                  key.length === 0
                    ? selectedKeys.length === 0
                    : key.length === selectedKeys.length &&
                      key.every((value) => selectedKeys.includes(value));
                return (
                  <button
                    type="button"
                    key={key.join("|") || "all"}
                    ...
                    className={`min-h-9 flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold font-inter border transition-colors whitespace-nowrap focus:outline-none focus-visible:ring-2 focus-visible:ring-[#238869] focus-visible:ring-offset-2 ${
                      isActive
  ...
  return (
    <div className="relative flex w-full lg:space-x-4">
