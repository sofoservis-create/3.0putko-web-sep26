...
// Scoped to this script's OWN fixtures (slug LIKE 'fx-%').
//
// Not a convenience — a correctness requirement. The first version of this
// helper matched every listing in the table, which passed only because the
// table happened to be empty. The moment scripts/seed-demo-listings.mjs
// had been run, eight membership assertions failed with real listings in
// the "actual" column: the assertions had been testing the database's
// contents, not the membership rule. A verification that depends on what
// else happens to be in the database verifies nothing.
//
// The invariant and catalogue-sanity sections further down deliberately do
// NOT scope, because those are claims about the whole table.
...
/** Unscoped — every listing in the table. The invariant is a claim about
    the real catalogue, not about this script's fixtures, so it must not
    filter. */
const allPageSlugs = (slug) =>
  client.query(
    `SELECT l.slug
       FROM destinations d JOIN listings l ON ${MEMBERSHIP}
      WHERE d.slug = $1`,
...
/** Slovak has three plural forms. Two-form English logic gives
    "10 ubytovania", which is broken Slovak — the same bug
    artifacts/web/app/_lib/format.ts exists to prevent, and which the
    first version of this script's own output committed. */
const listingsWord = (n) =>
  n === 1 ? "ubytovanie" : n >= 2 && n <= 4 ? "ubytovania" : "ubytovaní";
...
try {
  ...
  const FIXTURES = [
    ...
    // The Liptov/Orava boundary trio — see the assertions below.
    ["fx-zuberec", "Chalupa Zuberec", "published", 19.6167, 49.2603],
    ["fx-pribylina", "Drevenica Pribylina", "published", 19.7900, 49.1050],
    ["fx-vychodna", "Chata Východná", "published", 19.9167, 49.0667],
  ...
  const allDestPageSlugs = { liptov: await pageSlugs("liptov") };
  ...
  // Pribylina and Východná are here too, and belong: both sit directly
  // under the Západné/Vysoké Tatry on the Liptov side. Overlap is the
  // model working, not leaking.
  check("both are also in the parent, Tatry — with the two Liptov villages under the range",
    await pageSlugs("tatry"),
    ["fx-pribylina", "fx-strbske-pleso", "fx-tatranska-lomnica", "fx-vychodna"]);
  ...
  // Nízke Tatry's 30 km circle spans the range AND the Liptov valley on
  // its north side, so the two valley villages fall in as well. Same
  // effect as Liptov/Zuberec below: a circle centred on a ridge cannot
  // help covering both flanks.
  check("...and in Nízke Tatry, its parent, alongside the valley villages",
    await pageSlugs("nizke-tatry"),
    ["fx-demanovska", "fx-pribylina", "fx-vychodna"]);

  check("...and in Liptov, a different top-level destination that also contains it",
    await pageSlugs("liptov"),
    ["fx-demanovska", "fx-pribylina", "fx-vychodna", "fx-zuberec"]);
  ...
  // ── The circle model's hard limit, in executable form ─────────────────
  //
  // Liptov is a long east–west valley with Orava directly north of it.
  // Measured from Liptov's centre:
  //
  //     Bešeňová   6.6 km  (Liptov)
  //     Ružomberok 15.5 km (Liptov)
  //     Pribylina  19.8 km (Liptov)
  //     Zuberec    20.7 km (ORAVA)      ← between two Liptov places
  //     Východná   29.1 km (Liptov)
  //
  // Zuberec is nearer the centre of Liptov than Východná is, so NO radius
  // includes the real Liptov and excludes Zuberec. This is not a tuning
  // problem like Horehronie was — it is arithmetic, and it is the case
  // that says the long-valley regions eventually need polygons rather
  // than circles (docs/DESTINATIONS.md).
  //
  // Asserted rather than merely noted, so that the day someone swaps in
  // ST_Covers this test flips and says so. Until then it locks the
  // current, known-imperfect behaviour in place: a Zuberec chalupa DOES
  // appear under Liptov, and it is a product judgement whether that is
  // acceptable (it is ~40 min from Jasná, so it is not absurd).
  check("Zuberec is correctly in Orava",
    (await pageSlugs("orava")).includes("fx-zuberec"), true);

  check("KNOWN LIMITATION: Zuberec also lands in Liptov — no radius can prevent it",
    (await pageSlugs("liptov")).includes("fx-zuberec"), true);

  check("...while the genuinely-Liptov places on both sides of it are in too",
    ["fx-pribylina", "fx-vychodna"].every((s) => allDestPageSlugs.liptov.includes(s)),
    true);

  check("Košice contains only the Košice listing",
    await pageSlugs("kosice"), ["fx-kosice-centrum"]);

  check("Bratislava contains no fixture — 300 km from every one of them",
  ...
  check("rollback safety: this script's fixtures are all present and scoped",
    (await client.query(
      `SELECT count(*)::int AS n FROM listings WHERE slug LIKE 'fx-%'`
    )).rows[0].n, 10);
  ...
  for (const t of tiles) {
    const actual = (await allPageSlugs(t.slug)).length;
    if (actual !== t.n) {
  ...
  console.log("\nHome page grid, as it would render right now (fixtures included):\n");
  ...
  `);
  for (const r of grid) {
    console.log(`  ${r.name.padEnd(24)} ${r.n} ${listingsWord(r.n)}`);
  }
...
// Only this script's rows. Asserting the table is empty was wrong the
// moment real listings existed — and it would have "passed" forever on an
// empty database while silently proving nothing.
const left = (await client.query(
  "SELECT count(*)::int AS n FROM listings WHERE slug LIKE 'fx-%'"
)).rows[0].n;
check("rollback left none of this script's fixtures behind", left, 0);
